...
from sklearn.model_selection import learning_curve, GridSearchCV,  train_test_split, ShuffleSplit, cross_val_score
...
for ngram_scheme in ngram_schemes:
	...
	for i in range(len(vectorizers)):
		...
		cv = ShuffleSplit(len(y), test_size= 0.3, train_size=None)
		...
		gs_clf = GridSearchCV(estimator=clf, param_grid=parameters, n_jobs=-1, cv=cv, scoring='accuracy', verbose=2)
